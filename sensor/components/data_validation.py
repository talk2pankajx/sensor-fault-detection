...
class DataValidation:
    ...
    def drop_missing_values_column(self,df:pd.DataFrame,report_key_name:str )->Optional[pd.DataFrame]:
        try:
            ...
            null_report = (df.isnull().sum())*100/df.shape[0]
    ...
    def is_required_columns_exist(self,df:pd.DataFrame,report_key_name:str) ->bool:
            try:
                ...
                if len(missing_required_columns) ==0:
                    return True
                logging.info(f"missing required columns are {missing_required_columns} ")                
                self.validation_error[report_key_name] = missing_required_columns
                return False  
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
        try:
            ...
            train_df = self.drop_missing_values_column(df=train_df, report_key_name= "train_missing_values_columns")
            test_df = self.drop_missing_values_column(df=test_df, report_key_name= "test_missing_values_columns")
            ...
            is_exist = self.is_required_columns_exist(df=train_df, report_key_name="train_required_columns")
            if not is_exist:
                raise Exception("required columns are not available in train_df")
                        
            is_exist = self.is_required_columns_exist(df=test_df, report_key_name="test_required_columns")
            if not is_exist:
